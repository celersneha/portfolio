...
export async function GET() {
  ...
  try {
    ...
    const [stats, allTimeData]: [WakaTimeStatsData, WakaTimeAllTimeData] =
      await Promise.all([statsResponse.json(), allTimeResponse.json()]);
    ...
    const startDate = new Date(weeklyStats.start).toLocaleDateString(
      "en-US",
      dateFormatOptions
    );
    const endDate = new Date(weeklyStats.end).toLocaleDateString(
      "en-US",
      dateFormatOptions
    );
    const bestDayDate = new Date(weeklyStats.best_day.date).toLocaleDateString(
      "en-US",
      dateFormatOptions
    );
